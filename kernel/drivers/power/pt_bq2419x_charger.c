...
#define IINLIM_MIN				150*1000	// 150mA
...
static int bq2419x_iinlim_set(struct pt_bq2419x_chg_chip *chip, unsigned int uA)
{
	...
	/* to prevent entering HIZ state 
		iinlim 100mA -> iinlim 150mA
	*/
	if(mA < bq2419x_iinlim_val[/*IINLIM_100MA*/IINLIM_150MA])
		mA = bq2419x_iinlim_val[/*IINLIM_100MA*/IINLIM_150MA];
...
update_heartbeat_work(struct work_struct *work)
{
	...
	if(get_prop_batt_present(chip)) {
		...
		if(can_start_recharging(chip, sys_stat)) 
