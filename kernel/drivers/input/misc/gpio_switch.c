...
// ++ P16088 140306 LS4@Touch/HS - To set debounce time by user space for Hall IC function test.
#define GPIO_SWITCH_IOCTL_SET_BUMBPER_CASE_DEBOUNCE_TIME 0
// -- P16088
        ...
        case GPIO_SWITCH_IOCTL_SET_BUMBPER_CASE_DEBOUNCE_TIME:
            while(p) {
                if(strcmp(p->name , "sub_smart_cover1_detection") == 0) {
                    printk("%s : set %s debounce time -> %dms\n", __func__, p->name, (int)arg);
